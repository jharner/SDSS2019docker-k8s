...
These technologies form the basis for solving the data analysis issues mentioned above. Docker Swarm is also an orchestration option, but not as scalable or feature complete as Kubernetes. At the other extreme, Mesos is for huge containerization deployments.
...
- [Rocker](https://www.rocker-project.org): a suite of R-based images built by and hosted on Docker Hub

- [RSpark](https://github.com/jharner/rspark): a suite of R and Spark-based images hosted on Docker Hub
...
RSpark is being deployed to [Digital Ocean Kubernetes](https://www.digitalocean.com/products/kubernetes/).  

Kubernetes is now available on AWS, Azure, and of course Google Cloud as a service. However, these cloud providers do not have predictable price models and users/developers can incur large unexpected costs. Digital Ocean gives the user far more control over these costs, i.e., limits can be set and the user is only charged for what is actually used.

That said---it would be easy to deploy RSpark to any cloud provider using Kubernetes.

But why is Kubernetes necessary for RSpark? Primarily scalability and reliability. For example, the Spark cluster component can be scaled indefinitely based on the user's needs.
